...
function character_creator.set_base_textures(player)
	...
	character_creator.set_base_texture(player, base_texture)
end

function character_creator.colorize_texture(player, what, texture)
	local base_texture = character_creator.get_base_texture_table(player)
	if base_texture[what]["color"] then
		local value
		if what == "skin" then
			value = character_creator.skin_colors[base_texture[what]["color"]]
		else --"hair"
			value = character_creator.hair_colors[base_texture[what]["color"]]
		end
		if value then
...
function character_creator.compose_base_texture(player, def)
	local base_texture = character_creator.get_base_texture_table(player)
	local texture = character_creator.colorize_texture(player, "skin", "[combine:"..def.canvas_size..":0,0="..def.skin_texture)
	...
	for i = 1, #ordered_keys do
		...
		if key == "eyebrowns" then
			value.texture = character_creator.colorize_texture(player, "eyebrowns", value.texture)
		...
		elseif key == "hair" then
			...
			value.texture = character_creator.colorize_texture(player, "hair", value.texture)
